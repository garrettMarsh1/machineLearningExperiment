...
function newGenerations(){
    ...
    if(localStorage.getItem("bestTrafficRuns")){
        for(let i = 0; i < traffic.length; i++){
            ...
            if(i!=0){
                NeuralNetwork.mutate(traffic[i].brain, .9);
                }
            }
        }
}
...
function generateCars(N){
    let cars=[];
    for(let i =1;i<=N;i++){
        cars.push(new Car(road.getLaneCenter(2),100,50,70,"AI"));
...
//function to count and display the total number of cars which arent damaged and cars which are damaged
function countCars(){
    let count = 0;
    let count2 = 0;
    for(let i = 0; i < cars.length; i++){
 ...
 */
 function generateTraffic(N2){
    ...
    for(let i =1;i<=N2;i++){
        ...
        random(-40000, -300),50,70,"AI", random(3,5));
...
async function animate(time){
    ...
    //find best ten cars according to lowest y value
    topTenCars = cars.sort((a,b) => a.y - b.y).slice(0,10);
   ...
   //if all 
    ...
    if(cars.filter(c=>c.damaged).length>=cars.length){
