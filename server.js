...
app.get('/delete', async function(request, response) {
    ...
    // Prevent all other query types
    if (request.method !== 'DELETE') return response.json({ error: 'Only DELETE requests allowed' });

    console.log('Attempted to delete data from client');
    ...
    // For some fuck off reason, if this some how fails you have an edge case to tell people it didn't save
    if (!data) return response.status(500).send('Error while deleting data from the database');
